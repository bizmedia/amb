...
export async function sendMessage(storage, input) {
...
export function getInboxMessages(storage, projectId, agentId) {
    return storage.getInboxAndMarkDelivered(projectId, agentId);
}
export async function ackMessage(storage, projectId, messageId) {
...
export async function retryTimedOutMessages(storage, projectId) {
...
export function getDlqMessages(storage, projectId) {
    return storage.findMessages(projectId, { status: "dlq" });
}
export async function cleanupOldMessages(storage, projectId, retentionDays = 30) {
...
export async function retryDlqMessage(storage, projectId, messageId) {
    const message = await storage.getMessageById(projectId, messageId);
    if (!message)
        throw new NotFoundError("Message");
...
export async function retryAllDlqMessages(storage, projectId) {
    const retried = await storage.updateManyMessagesToStatus(projectId, { status: "dlq" }, { status: "pending", retries: 0 });
    return { retried };
}
